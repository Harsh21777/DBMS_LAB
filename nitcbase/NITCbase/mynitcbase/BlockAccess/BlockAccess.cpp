...
RecId BlockAccess::linearSearch(int relId, char attrName[ATTR_SIZE], union Attribute attrVal, int op) {
  ...
  //   RelCatEntry relCatBuff;
    ...
    while (block != -1)
    {
        ...
        */
        Attribute rec[blockHead.numAttrs];
        
        blockBuff.getRecord(rec,slot);
        AttrCatEntry attrCatBuff;
        AttrCacheTable::getAttrCatEntry(relId,attrName,&attrCatBuff);
        /* use the attribute offset to get the value of the attribute from
           current record */
        int attrOff=attrCatBuff.offset;
        

        int cmpVal;  // will store the difference between the attributes
        // set cmpVal using compareAttrs()
        cmpVal=compareAttrs(rec[attrOff],attrVal,attrCatBuff.attrType);
