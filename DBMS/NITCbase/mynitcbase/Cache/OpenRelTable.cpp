  ...
  while(entry)
  {
      if(entry->dirty==true)
      {
        /* Get the Attribute Catalog entry from attrCache
         ...
         AttrCatEntry atbEntry;
         AttrCacheTable::getAttrCatEntry(relId,entry->attrCatEntry.offset,&atbEntry);
         Attribute recor[ATTRCAT_NO_ATTRS];
         AttrCacheTable::attrCatEntryToRecord(&atbEntry,recor);
         RecBuffer buff(entry->recId.block);
         buff.setRecord(recor,entry->recId.slot);
