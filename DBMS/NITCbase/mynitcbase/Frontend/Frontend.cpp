                                               ...
                                               char attribute[ATTR_SIZE], int op, char value[ATTR_SIZE])
{
  ...
  char temp[] = ".temp";
  int check = Algebra::select(relname_source, temp, attribute, op, value);
  if (check != SUCCESS)
  {
    return check;
  }

  relId tempId = OpenRelTable::openRel(temp);
  ...
  check = Algebra::project(temp, relname_target, attr_count, attr_list);
