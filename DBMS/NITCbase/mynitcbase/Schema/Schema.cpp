...
int Schema::renameRel(char oldRelName[ATTR_SIZE], char newRelName[ATTR_SIZE]) {
  // if the oldRelName or newRelName is either Relation Catalog or Attribute Catalog,
    ...
    if(strcmp(RELCAT_RELNAME,oldRelName)==0||
       strcmp(RELCAT_RELNAME,newRelName)==0||
       strcmp(ATTRCAT_RELNAME,oldRelName)==0||
       strcmp(ATTRCAT_RELNAME,newRelName)==0
    ){
      return E_NOTPERMITTED;
    }
...
int Schema::createRel(char relName[],int nAttrs, char attrs[][ATTR_SIZE],int attrtype[]){
  ...
  Attribute relNameatb;
  strcpy((char*)relNameatb.sVal,(const char*)relName);
  char rname[]="RelName";
  ...
  targetRec=BlockAccess::linearSearch(RELCAT_RELID,rname,relNameatb,EQ);
  if(targetRec.block!=-1||targetRec.slot!=-1){
    return E_RELEXIST;
  }
  ...
  for(int i=0;i<nAttrs;i++)
  {
      ...
      Attribute atbCatRec[ATTRCAT_NO_ATTRS];
      strcpy(atbCatRec[ATTRCAT_REL_NAME_INDEX].sVal,relName);
      strcpy(atbCatRec[ATTRCAT_ATTR_NAME_INDEX].sVal,attrs[i]);
      atbCatRec[ATTRCAT_ATTR_TYPE_INDEX].nVal=attrtype[i];
      atbCatRec[ATTRCAT_PRIMARY_FLAG_INDEX].nVal=-1;
      atbCatRec[ATTRCAT_ROOT_BLOCK_INDEX].nVal=-1;
      atbCatRec[ATTRCAT_OFFSET_INDEX].nVal=i;

      retVal=BlockAccess::insert(ATTRCAT_RELID,atbCatRec);
      if(retVal!=SUCCESS){
...
int Schema::deleteRel(char *relName) {
  ...
  //     return E_NOTPERMITTED
      // (check if the relation names are either "RELATIONCAT" and "ATTRIBUTECAT".
      // you may use the following constants: RELCAT_RELNAME and ATTRCAT_RELNAME)
  if(strcmp(ATTRCAT_RELNAME,relName)==0||strcmp(RELCAT_RELNAME,relName)==0){
    return E_NOTPERMITTED;
  }
...
int Schema::createIndex(char relName[ATTR_SIZE],char attrName[ATTR_SIZE]){
  ...
  relId RelId=OpenRelTable::getRelId(relName);
  if(RelId==E_RELNOTOPEN){
    return E_RELNOTOPEN;
  }
