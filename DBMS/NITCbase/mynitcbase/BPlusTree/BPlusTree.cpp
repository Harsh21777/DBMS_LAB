...
int BPlusTree::bPlusCreate(int relId, char attrName[ATTR_SIZE])
{
  ...
  int check = AttrCacheTable::getAttrCatEntry(relId, attrName, &attrEntry);
  if (check != SUCCESS)
  {
  ...
  while (block != -1)
  {
    ...
    RecBuffer buffs(block);

    unsigned char slotMap[relCatEntry.numSlotsPerBlk];
    buffs.getSlotMap(slotMap);
    ...
    for (int i = 0; i < relCatEntry.numSlotsPerBlk; i++)
    {
      if (slotMap[i] == SLOT_OCCUPIED)
      {
        ...
        buffs.getRecord(record, i);
        RecId rec_id = {block, i};
        ...
        // int offset = attrEntry.offset;
        check = BPlusTree::bPlusInsert(relId, attrName, record[attrEntry.offset], rec_id);
...
int BPlusTree::bPlusDestroy(int rootBlockNum)
{
  ...
  if (type == IND_LEAF)
  {
    ...
    // constructor
    IndLeaf leafBuff(rootBlockNum);
    leafBuff.releaseBlock();
  ...
  else if (type == IND_INTERNAL)
  {
    ...
    // constructor
    IndInternal internBuff(rootBlockNum);
    HeadInfo head;
    internBuff.getHeader(&head);
    ...
    InternalEntry inter_entry;
    internBuff.getEntry(&inter_entry, 0);

    if (inter_entry.lChild != -1)
    {
      int ret = bPlusDestroy(inter_entry.lChild);
      if (ret != SUCCESS)
        return ret;
    }
    for (int entry = 0; entry < head.numEntries; entry++)
    {
      internBuff.getEntry(&inter_entry, entry);
      if (inter_entry.rChild != -1)
      {
        int check = BPlusTree::bPlusDestroy(inter_entry.rChild);
        if (check != SUCCESS)
        {
          return check;
        }
...
int BPlusTree::bPlusInsert(int relId, char attrName[ATTR_SIZE], Attribute attrVal, RecId recId)
{
  ...
  int leafBlkNum = BPlusTree::findLeafToInsert(blockNum, attrVal, attrEntry.attrType);
  // IndLeaf leafBuff(leafBlkNum);
  Index entry;
  entry.attrVal = attrVal;
  entry.block = recId.block;
  entry.slot = recId.slot;

  // leafBuff.setEntry(&entry,0);
  check = BPlusTree::insertIntoLeaf(relId, attrName, leafBlkNum, entry);
...
int BPlusTree::createNewRoot(int relId, char attrName[ATTR_SIZE], Attribute attrVal, int lChild, int rChild)
{
  ...
  IndInternal newRootBuff;

  int newRootBlkNum = newRootBuff.getBlockNum();
  ...
  HeadInfo head;
  newRootBuff.getHeader(&head);
  head.numEntries = 1;
  newRootBuff.setHeader(&head);
  ...
  newRootBuff.setEntry(&newEntry, 0);
  ...
  BlockBuffer lftBuff(lChild);
  BlockBuffer rytBuff(rChild);

  HeadInfo lftHead, rytHead;
  lftBuff.getHeader(&lftHead);
  rytBuff.getHeader(&rytHead);

  lftHead.pblock = newRootBlkNum;
  rytHead.pblock = newRootBlkNum;

  lftBuff.setHeader(&lftHead);
  rytBuff.setHeader(&rytHead);
